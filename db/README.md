...
| # | 规约 | 理由 |
|---|---|---|
| 1 | 表名**单数**、`snake_case`、**不使用保留字** | `user` / `group` 是保留字，被迫加引号后每处引用都要记得加 |
...
| 4 | 每张业务表带 `tenant_id uuid NOT NULL REFERENCES tenant(id)` | 见 Phase 0 §9.1：当前单租户，但预留位置。事后加列要动全部外键 |
| 5 | 金额一律 `numeric(14,2)`，单位**元**，**禁止浮点** | 万元 + float 是对账对不平的经典成因。展示层再转万元 |
