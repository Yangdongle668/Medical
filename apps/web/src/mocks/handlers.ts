...
import { makeScenario, SITES_LIST, STAFF_LIST, SITE_STAFF, FUNNELS, AUDIT_ENTRIES,
  ASSIGNMENTS, newAssignmentId, mkStaff, mkTimesheet, WORK_TYPE_META, nextOf,
...
export const scenarioHandlers = [
  ...
  http.post(pathToRegExp("/v1/subject-visits/{id}:edc-entered"), ({ request }) => {
    const id = seg(request.url, /\/subject-visits\/([^/:]+):edc-entered/);
    const v = scenario.visits.find(x => x.id === id);
    if (!v) return HttpResponse.json(
      problem("not-found", 404, "访视不存在"), { status: 404 });
    v.edcStatus = "entered"; v.edcDaysLate = null;
    return HttpResponse.json({ data: v, sideEffects: [
      { type: "EdcEntered", summary: `${v.visitLabel} 已标记录入 EDC` }
    ] }, { status: 201 });
  }),
  ...
  /* 登记一条 SAE。**发生时刻由请求带来，不是 now()** ——
     两者混为一谈，及时率就永远是 100%。 */
  http.post(pathToRegExp("/v1/study-sites/{id}/sae"), async ({ request }) => {
    const id = seg(request.url, /\/study-sites\/([^/]+)\/sae/);
    const b = await request.json() as {
      title: string; detail: string; occurredAt: string; reportedAt?: string;
    };
    if (!identity().actions.includes("subjWrite")) return HttpResponse.json(
      problem("forbidden", 403, "你的角色不能登记 SAE"), { status: 403 });
    const site = SITES_LIST.find(x => x.id === id);
    if (!site) return HttpResponse.json(
      problem("not-found", 404, "中心不存在"), { status: 404 });

    const n = scenario.qualityEvents.filter(q => q.kind === "sae").length;
    const row = {
      id: `qe-sae-${n + 1}`, code: `SAE-2026-${String(10 + n).slice(-3)}`,
      siteCode: site.code, studySiteId: site.id,
      kind: "sae", severity: "critical", state: "open",
      title: b.title, detail: b.detail,
      occurredAt: b.occurredAt, reportedAt: b.reportedAt ?? null,
      autoGenerated: false, raisedBy: identity().role.code, ageDays: 0,
      sourceEventId: null
    } as unknown as (typeof scenario.qualityEvents)[number];
    scenario.qualityEvents.unshift(row);
    return HttpResponse.json(row, { status: 201 });
  }),

  /* 登记「已上报」。**超过 24 小时的，同一个事务里生成一条 sae_late** ——
     不可跳过、不能人工删除，只能整改关闭（I6）。 */
  http.post(pathToRegExp("/v1/quality-events/{id}:sae-reported"), async ({ request }) => {
    const id = seg(request.url, /\/quality-events\/([^/:]+):sae-reported/);
    const b = await request.json() as { reportedAt: string };
    if (!identity().actions.includes("subjWrite")) return HttpResponse.json(
      problem("forbidden", 403, "你的角色不能登记 SAE 上报"), { status: 403 });
    const e = scenario.qualityEvents.find(q => q.id === id);
    if (!e) return HttpResponse.json(
      problem("not-found", 404, "质量事件不存在"), { status: 404 });
    if (e.reportedAt) return HttpResponse.json(problem("invariant-violated", 422,
      "这一条已经登记过上报了"), { status: 422 });

    e.reportedAt = b.reportedAt;
    const hours = saeReportHours({ occurredAt: e.occurredAt!, reportedAt: b.reportedAt });
    const effects: { type: string; summary: string }[] = [];
    if (hours !== null && hours > 24) {
      /* 自动生成的那一条：**不是事后补录，是同一个事务里写的**。 */
      const n = scenario.qualityEvents.filter(q => q.kind === "sae_late").length;
      scenario.qualityEvents.unshift({
        ...e,
        id: `qe-late-${n + 1}`, code: `PD-2026-${String(60 + n).slice(-3)}`,
        kind: "sae_late", severity: "major", state: "open",
        title: `${e.code} 超过 24 小时上报`,
        detail: `发生至上报经过 ${hours.toFixed(1)} 小时，超出 24 小时时限。`,
        autoGenerated: true, sourceEventId: e.id
      } as unknown as (typeof scenario.qualityEvents)[number]);
      effects.push({ type: "SaeLateRaised",
        summary: `超时 ${(hours - 24).toFixed(1)} 小时 —— 已自动生成一条超时质量事件，` +
          "它不能人工删除，只能整改关闭" });
    } else {
      effects.push({ type: "SaeReported", summary: `${e.code} 已按时上报` });
    }
    return HttpResponse.json({ data: e, sideEffects: effects }, { status: 201 });
  }),
  ...
  http.post(pathToRegExp("/v1/subjects/{id}:sign-icf"), async ({ request }) => {
    ...
    /* ── 筛选期访视要**真的排出来** ──────────────────────────────────
       这里原来只把计数改成 `visitsPlanned = 8`，一条访视都不建 ——
       于是 mock 上每一个刚签完知情的人，都是一个**进得去出不来**的人：
       入组闸门要求筛选期访视已登记 PI 确认，而他连访视都没有；
       受试者访视窗口上那一行只有「登记脱落」。
  ...
  http.post(pathToRegExp("/v1/subjects/{id}:withdraw"), async ({ request }) => {
    const sub = subjectFrom(request, /\/subjects\/([^/:]+):withdraw/);
    if (!sub) return notFoundSubject();
    const b = await request.json() as
      { reason: string; withdrawnOn: string; note: string };
    if (!identity().actions.includes("subjWrite")) return HttpResponse.json(
      problem("forbidden", 403, "你的角色不能登记脱落"), { status: 403 });

    const remaining = Math.max(0, sub.visitsPlanned - sub.visitsDone);
    sub.state = "withdrawn"; sub.withdrawReason = b.reason;
    sub.exitedOn = b.withdrawnOn;
    /* **剩余访视一并作废** —— 不作废的话这一例会永远刷红超窗。 */
    sub.nextVisit = null;
    return HttpResponse.json({ data: maskSubject(sub), sideEffects: [
      { type: "VisitsVoided",
        summary: `剩余 ${remaining} 次未完成访视已作废 —— 否则这一例会永远刷红超窗` },
      { type: "RevenueProrated",
        summary: `收入按已完成访视比例计：${sub.visitsDone}/${sub.visitsPlanned}，不按整例（I8′）` }
    ] }, { status: 201 });
  }),
 ...
 *  `getDate()/setDate()` 却按本地时区算，再用 `toISOString()` 切回日期。
 *  东八区看不出问题，负时区那个 Date 落在**前一天**下午，加完差一天。
 *  服务端的 `scheduleVisit` 与 packages/calc 的 `workdaysBetween`
 *  犯过同一个错，修法一样。 */
