...
/* ════════════════════════════════════════════════════════════════════
   ...
   **「发起」那一端没接** —— 能判定一份立项申请但提不了，
   ...
   ④ **提交条吸底。** 展开之后表单往往比一屏还高 —— 1440×900 上实测：
      合同变更 732px、可行性 648px，提交按钮分别落在 1213px 与 1144px，
      也就是**要往下滚三百来像素才够得着**（七张表里有三张这样；
      换到更常见的 1366×768 就是五张）。
      填到一半看不见"填完按哪儿"，人只好先滚下去确认按钮还在。
      所以那一行贴着视口底部走，连同旁边那句"这一下会引起什么"。
   ════════════════════════════════════════════════════════════════════ */
    ...
    <section className={`card${wide ? "" : ""}`} data-testid={`${testid}-form`}
      ...
      <div className="card-b">
        <div className="stack">
          ...
          {children}
          {/* 吸底那一行。`position: sticky; bottom: 0` —— 表单比一屏矮时
              它就待在原地，和以前一模一样；高过一屏才开始跟着视口走。
              所以这不是"给长表单加的特例"，是同一条规则的两端。 */}
          <div className="row form-go">
