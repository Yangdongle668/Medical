...
export function Rail({ groups, here, split = null }: {
  ...
  return (
    <nav>
      ...
      {/* ── 一个模块都没有 ────────────────────────────────────────────
          迁移 0051 之后这是一个**真的会出现**的状态：机构办与研究者
          两个外部角色默认不带任何页面。

          在此之前这里什么都不画 —— 侧栏只剩一个品牌名，底下一片空白。
          而空白说不出自己是哪一种空白：是权限被收走了、是还在加载、
          还是系统坏了？三种猜法里没有一种是对的，**而正确答案
          （"这个角色默认就不开"）一个字都没出现在屏幕上。**

          所以照实说，并且说清楚下一步在谁手上。 */}
      {!needle && groups.length === 0 && (
        <p className="nav-none" data-testid="rail-empty">
          <b>这个角色默认没有开通任何页面。</b>
          本系统是对内的 —— 院方的机构办与研究者不必登录，
          他们经手的事由院内的人登记进来。
          <br />
          真要给某家医院开，管理员在<b>「组织与权限」</b>里把模块勾回来即可，
          不用改代码。
        </p>
      )}
    </nav>
