...
test.describe("导航与页面一一对应", () => {
  ...
  /* CRC 从 14 条变成 15 条：迁移 0052 把「立项受理」（`instac`）给了他 ——
     闸门叫他去那一页登记受理意见函，而在那之前他的侧栏上没有那一页。
     **这个数写死是有意的**：它一变就说明有人动了某个角色的模块清单，
     而那件事该有人看一眼，不该悄悄过去。 */
  for (const [role, count] of [["crc", 15], ["pm", 14], ["boss", 19]] as const) {
