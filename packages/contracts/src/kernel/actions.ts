...
export const ACTION_LABEL: Record<ActionKey, string> = {
  ...
  manage: "管理人员与权限",
  monitor: "排期与执行监查访视",
  /** **「登记」这两个字是整条语义。** 原来叫「PI 确认访视」，而那个说法
   *  暗示的是"PI 本人登录进来点一下" —— 于是这个动作只给了外部的 pi 角色，
   *  而 15 个中心里 14 个没绑 PI 账号，那些中心的访视永远推不动
   *  （实测 189 条卡住，且不计入「已完成」统计，没有任何地方报错）。
   *  PI 签的字仍然是放行条件，只是那件事**由一线带着日期登记进来** ——
   *  与「登记伦理批复」「登记立项材料递交」同一个形状。 */
  piConfirm: "登记 PI 确认访视",
  raiseQ: "发起数据质疑",
  rateWrite: "维护费率卡",
  subjRead: "查看受试者明细",
  subjWrite: "登记受试者与访视",
