...
/* ════════════════════════════════════════════════════════════════════
   一线履职 —— **不是 Staff 的一个筛子，是另一个问题的答案。**

   `Staff` 回答「这个人在我方是什么情况」：职级、城市、带教、继任、
   带几个中心、为什么停用。那是人事账，半年才变一次。
   这里回答的是**这周**的事：他该登记的几件，登记了没有。

   ── 为什么它现在才需要存在 ────────────────────────────────────────
   迁移 0048 与 0050 把立项受理与 PI 确认从「等院外的人点一下」改成了
   「由院内的人带着日期登记进来」。那一步把风险换了个地方：
   ...
   在此之前没有一页回答得了：团队工作台按中心排（"SS-07 怎么样了"），
   经营驾驶舱按问题类型排，两者都不按人排。

   ── 行范围照旧 ────────────────────────────────────────────────────
   这几个数是在调用者自己的行范围内数出来的（RLS 原样生效）：
   PM 看到的是本组的，经营层看到的是全部，外部方一行都没有
   （`staff_scope` 对他们整表关闭）。**所以它不需要一个新动作**，
   也不需要一张新表 —— 它只是把已有的行按人归了一次。
   ════════════════════════════════════════════════════════════════════ */
export const RegistrationDuty = z.object({
  accountId: Uuid,
  login: z.string(),
  displayName: z.string(),
  roleKind: RoleKind,
  /** 访视做完了，PI 签字那件事还没登记（状态停在 `done_pending_pi`）。
   *  **这一类不计入「已完成」统计** —— 它欠着的时候，
   *  这个中心的入组进度、完成率、成本归集全是偏低的。 */
  pendingPiConfirm: z.int(),
  /** 访视完成超过 5 个工作日还没标记录入 EDC。不阻断，但进及时率统计。 */
  edcOverdue: z.int(),
  /** 窗口已经关了而访视还没完成 —— **每一条都是一次方案偏离**。 */
  outOfWindow: z.int(),
  /** 立项材料递交了、受理意见函还没登记。核查要看的是那张纸。 */
  acceptanceNoLetter: z.int(),
  total: z.int(),
  /** 最久的那一件挂了多少天。**排序用的是它，不是件数** ——
   *  按件数排，带三个大中心、每样欠一点的人会永远第一，
   *  而真正该先处理的是那条挂了四十天的：它已经不是"来不及"，是"忘了"。
   *  一件都不欠时为 null。 */
  oldestDays: z.int().nullable()
}).meta({
  id: "RegistrationDuty",
  description:
    "**只收这个人现在就办得掉的事。** 等中心回复的质疑、等排期的监查、" +
    "要别人审批的工时一律不收 —— 把「在等别人」混进「你欠着」，" +
    "这张表会立刻失去说服力，而一张会冤枉人的清单，人只会学会忽略它。"
});
